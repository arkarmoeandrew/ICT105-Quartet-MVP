...
function renderConversationList() {
  ...
  const filtered = conversations.filter((item) => `${item.other?.display_name || ""} ${item.listing_title || ""}`.toLowerCase().includes(query));
  if (!filtered.length) {
    conversationList.innerHTML = `<div class="empty-state"><h3>No matches</h3><p>Try a different name or listing.</p></div>`;
    return;
  }
  conversationList.innerHTML = filtered.map((item) => {
    const unread = !item.last_read_at || new Date(item.updated_at) > new Date(item.last_read_at);
...
async function selectConversation(id) {
  ...
  chatContext.textContent = conversation.listing_title || "Private marketplace conversation";
