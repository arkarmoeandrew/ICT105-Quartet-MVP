...
| **FR-03** | **User input or data submission feature** | The system must allow students to submit information by creating a resource, equipment, or student service listing with details such as title, description, category, availability, and contact information. |
| **FR-04** | **Data storage or record management** | Submitted listings and request information must be stored, displayed, or simulated using a suitable method such as local storage, JSON data, Supabase, Firebase, Google Sheet, or another database-backed approach. |
...
| **FR-08** | **Status or progress tracking** | The system must show the status of each listing or request, such as Available, Borrowed, Pending, Unavailable, Approved, or Rejected. |
...
| --- | --- |
| Web app | HTML, CSS, JavaScript, and optional Supabase or Firebase integration for authentication, database management, and request handling. |
